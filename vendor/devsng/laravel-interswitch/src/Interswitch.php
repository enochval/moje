...
class Interswitch
{
    ...
    protected $action = 'https://sandbox.interswitchng.com/collections/w/pay';
    ...
    public function form(array $options = [], string $id, $submitBtn = true)
    {
        ...
        if ($submitBtn) {
            ...
            $form .= '<input class="submit btn btn-blue btn-sm icon-left pull-left tooltip-primary" type="submit" name="submit" data-toggle="tooltip" data-placement = "right" data-original-title="Proceed To Interswitch" value="Proceed To Interswitch" />'.PHP_EOL;
        }
    ...
    {
        $hash = openssl_digest($str, "sha512");
        $this->hash = $hash;
    ...
    {
        $this->payitemid = /*config('interswitch.pay_item_id')*/101;
    }

    private function setProductID()
    {
        $this->productid = /*config('interswitch.product_id')*/1076;
    }
